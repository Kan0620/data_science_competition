...
def various_f(index,raw_df,key,about,n_unique):
    ...
    if index==0:
        ...
        raw_df['agg_'+about+'_by_'+key+'_median']=np.zeros(len(raw_df))
        
        for group in range(n_unique):
            
            x=raw_df[raw_df[key]==group][about]
            
            raw_df['agg_'+about+'_by_'+key+'_median'][raw_df[key]==group]=x.median()
        ...
        raw_df['agg_'+about+'_by_'+key+'_mm']=np.zeros(len(raw_df))
        
        for group in range(n_unique):
            
            x=raw_df[raw_df[key]==group][about]
            
            raw_df['agg_'+about+'_by_'+key+'_mm'][raw_df[key]==group]=(x.mean()-x.median())**2
        
        return raw_df
    
    elif index==9:
        
        raw_df['agg_'+about+'_by_'+key+'_minmax_norm']=np.zeros(len(raw_df))
        
        for group in range(n_unique):
            
            x=raw_df[raw_df[key]==group][about]
            
            raw_df['agg_'+about+'_by_'+key+'_minmax_norm'][raw_df[key]==group]=(x-x.min())/(x.max()-x.min())
...
def categorical_pop(x=int()):
    
    for i in range(7):
        
        if i*12<=x and x<=(i+1)*12-1:
...
def create_data():
    ...
    norm_list0=['popularity','duration_ms','acousticness','positiveness','danceability','loudness',
               'energy','liveness','speechiness','instrumentalness','fast','slow']
    
    norm_list1=['{}_prob'.format(i) for i in range(11)]+['{}_d'.format(i) for i in range(11)]
    
    all_data[norm_list0]=(all_data[norm_list0]-all_data[norm_list0].mean())/all_data[norm_list0].std()
    
    all_data[norm_list1]=(all_data[norm_list1]-all_data[norm_list1].mean())/all_data[norm_list1].std()
    ...
    for agg_key in agg_list:
        ...
        for column in norm_list0:
            ...
            if agg_key=='tempo':
                ...
                method_list=[2,3,4,5,6,8,9]
        
            agg_trans(all_data,agg_key,column,method_list)
            
    for agg_key in agg_list:
        
        for column in norm_list1:
...
instrumentalness    2.891089 +log
    ...
    train.to_csv('processed_train9.csv')
    test.to_csv('processed_test9.csv')
