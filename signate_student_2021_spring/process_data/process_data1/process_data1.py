...
def create_data():
    ...
    print(str(all_data.iloc[332,3])=='nan')
...
instrumentalness    2.891089 +log
    ...
    norm_list=['popularity','duration_ms','acousticness','positiveness','danceability','loudness',
               'energy','liveness','speechiness','instrumentalness','tempo_num']
    
    all_data[norm_list]=(all_data[norm_list]-all_data[norm_list].mean())/all_data[norm_list].std()
    
    #all_data[norm_list]=(all_data[norm_list]-all_data[norm_list].min())/(all_data[norm_list].max()-all_data[norm_list].min())
    ...
    train.to_csv('processed_train1.csv')
    test.to_csv('processed_test1.csv')
